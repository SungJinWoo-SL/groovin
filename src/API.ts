    ...
    */
    ...
    async getMangaDetails(source: Source, ids: string[]): Promise<Manga[]> {
        ...
        for (let request of requests) {
            ...
            manga.push(
                ...source.getMangaDetails(response.data, request.metadata)
            );
        }
    ...
    async getChapters(source: Source, mangaId: string): Promise<Chapter[]> {
        ...
        try {
            ...
            let chapters: Chapter[] = source.getChapters(
                data.data,
                request.metadata
            );
            return chapters;
    ...
    async filterUpdatedManga(
        ...
        while (hasResults && data.data) {
            let results: any = source.filterUpdatedManga(
                data.data,
                request.metadata
            );
    ...
    private async makeFilterRequest(
        ...
        let post: boolean =
            request.method.toLowerCase() == "post" ? true : false;
        try {
            if (!post) {
                request.url =
